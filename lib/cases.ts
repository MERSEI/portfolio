...
export const cases: Case[] = [
  {
    ...
    ru: {
      ...
      outcome:
        "Прогон на живом железе вскрыл то, чего не показали тесты. WASAPI loopback молчит буквально: пока через устройство вывода ничего не играет, поток не отдаёт буферов вообще, и дорожка собеседника получалась длиной в один WAV-заголовок — реплика с третьей минуты легла бы в позицию 0, и общая ось времени существовала только на бумаге. Пропуски теперь добиваются тишиной по монотонным часам: на живой записи дорожки расходятся на 15 мс за 12 секунд. Там же выяснилось, что дефолтный выбор устройства падал из коробки — видеокарта видна через драйвер, а считает через cuBLAS, который ставится отдельно, причём конструктор модели проходит молча и падает первое вычисление. Устройства перебираются по плану с прогревом на секунде тишины. 35 тестов, 9.9× реального времени на 12 ядрах; сбой разбора больше не уносит с собой запись.",
    },
    en: {
      ...
      outcome:
        "Running it on real hardware exposed what the tests could not. WASAPI loopback goes silent literally: while nothing plays through the output device the stream yields no buffers at all, so the other party's track came out as a lone WAV header — a line spoken in the third minute would have landed at position 0, and the shared time axis existed only on paper. Gaps are now padded with silence against a monotonic clock: on a live recording the tracks drift 15 ms over 12 seconds. The same run showed the default device choice crashing out of the box — the GPU is visible through the driver but computes through cuBLAS, which installs separately, and the model constructor passes silently while the first computation fails. Devices are now tried in order, each warmed up on a second of silence. 35 tests, 9.9× realtime on 12 cores; a failed analysis no longer takes the recording down with it.",
    },
  },
  {
    slug: "pulse",
    ...
    ru: {
      ...
      outcome:
        "Политика проверена не на моках, а на шести реальных адресах: первый отказ проходит молча, второй открывает инцидент, третий давится дедупом, восстановление инцидент закрывает — живые сервисы всё это время молчат, как и должны. Отдельно вскрылся барьер в порядке действий: разовый прогон требовал Telegram-токен, то есть проверить «вижу ли я вообще свои сервисы» нельзя было, не заведя сначала бота. Теперь он работает без единого токена. 40 тестов, CI на Python 3.11–3.13. Падение триажа не роняет алерт — сообщение о том, что сервис лежит, важнее объяснения почему; перезапуск деплоя из чата требует отдельного подтверждения.",
    },
    en: {
      ...
      outcome:
        "The policy was exercised against six real addresses rather than mocks: the first failure passes in silence, the second opens an incident, the third is swallowed by deduplication, and recovery closes it — while the live services stay quiet throughout, as they should. A separate barrier surfaced in the order of operations: a one-off run demanded a Telegram token, so \"can I even see my services\" could not be answered without first creating a bot. It now runs with no token at all. 40 tests, CI on Python 3.11–3.13. A failing triage never drops the alert — knowing the service is down matters more than knowing why; restarting a deployment from chat takes a separate confirmation.",
    },
  },
  {
